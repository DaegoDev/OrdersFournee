(function() {
  ...
  function($scope, $log, $state, $ngConfirm, ClientSvc, OrderService, $stateParams, ProfileService) {
    ...
    $scope.changedInitial = function() {
      ...
      if (($scope.order.timeInitial.getHours() > 1 && $scope.order.timeInitial.getHours() < 9)) {
        $scope.order.timeInitial = new Date();
        $scope.order.timeInitial.setHours(9);
        $scope.order.timeInitial.setMinutes(0);
        // console.log($scope.order.timeInitial);
      } else if ($scope.order.timeInitial.getHours() > 16) {
        $scope.order.timeInitial = new Date();
        $scope.order.timeInitial.setHours(16);
        $scope.order.timeInitial.setMinutes(0);
      } else if ($scope.order.timeInitial.getHours() > ($scope.order.timeFinal.getHours() - 2)) {
        $scope.order.timeFinal = new Date();
        $scope.order.timeFinal.setHours($scope.order.timeInitial.getHours() + 2);
        $scope.order.timeFinal.setMinutes($scope.order.timeInitial.getMinutes());
      }
    ...
    $scope.changedFinal = function() {
      if ($scope.order.timeFinal.getHours() > 18) {
        $scope.order.timeFinal = new Date();
        $scope.order.timeFinal.setHours(18);
        $scope.order.timeFinal.setMinutes(0);
      } else if (($scope.order.timeFinal.getHours() > 1 && $scope.order.timeFinal.getHours() < ($scope.order.timeInitial.getHours() + 2)) ) {
        $scope.order.timeFinal = new Date();
        $scope.order.timeFinal.setHours($scope.order.timeInitial.getHours() + 2);
        $scope.order.timeFinal.setMinutes($scope.order.timeInitial.getMinutes());
      }
