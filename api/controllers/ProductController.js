...
module.exports = {
  ...
  create: function(req, res) {
    ...
    Element.find().sort('id ASC')
      ...
      .then(function(itemProduct) {
        ...
        res.created({itemProduct: itemProduct});
      })
      .catch(function(err) {
        sails.log.debug(err);
   ...
   */
   update: function (req, res) {
    ...
    if (!unitsPack) {
      return res.badRequest({code:1, msg: 'Missing units pack parameter'});
    }

     if (!productCode) {
       return res.badRequest({code:1, msg: "Mising productCode parameter"});
     }
     ...
     if (typeof items == 'string') {
       ...
       items.forEach(function(item, i, items) {
        //  item = JSON.parse(item);
         ...
         index = addedItems.indexOf(items[i].name.toUpperCase().trim());
         if (index != -1) {
           return res.badRequest({code:3, msg: 'There are repeated elements.'})
         } else {
           addedItems.push(items[i].name.toUpperCase().trim());
         }
       })
     ...
     Element.find().sort('id ASC')
       ...
       .then(function (data) {
         ...
         if (!isValid) {
           throw {code: 4, msg: 'The dough must be de same as the original product.'}
         }

         return sql.beginTransaction();
       })
       .then(function () {
         return sql.delete('item_product', {product_code: productCode});
       })
       .then(function (data) {
         var itemProductCredentials = [];
       ...
       .then(function (data) {
         ...
         {name: productCredentials.name, short_name: productCredentials.shortName, price: productCredentials.price, units_pack: productCredentials.unitsPack},
       ...
       .then(function (data) {
         ...
         return res.ok({code:0, msg:"Item updated"});
       })
       .catch(function (err) {
